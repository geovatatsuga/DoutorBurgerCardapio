-- Allow storefront visitors to place orders without creating customer accounts.
-- Admin access remains protected by store_memberships and RLS policies.
...
select burgerc.id, d, '18:00'::time, '23:30'::time, d <> 1
from burgerc
...
select burgerc.id, c.name, c.icon, c.sort_order
from burgerc
cross join (values
  ('Burgers', 'burger', 10),
  ('Combos', 'combo', 20),
...
cats as (
  ...
  join burgerc on burgerc.id = c.store_id
)
insert into public.products (store_id, category_id, name, description, price_cents, image_path, is_favorite, is_combo, sort_order)
select burgerc.id, cats.id, p.name, p.description, p.price_cents, p.image_path, p.is_favorite, p.is_combo, p.sort_order
from burgerc
join (values
  ('Burgers', 'Doutor Burger', 'Pao brioche, blend 180g, cheddar, bacon, alface, tomate e molho especial.', 3490, '/assets/new-direction/doutor-burger.webp', true, false, 10),
  ('Burgers', 'Smash Cheddar', 'Dois smash burgers, cheddar cremoso, picles e cebola.', 2890, '/assets/new-direction/smash-cheddar.webp', true, false, 20),
  ('Combos', 'Combo Doutor', 'Doutor Burger, batata crocante e refrigerante gelado.', 4990, '/assets/new-direction/chicken-crispy.webp', true, true, 30)
) as p(category_name, name, description, price_cents, image_path, is_favorite, is_combo, sort_order) on true
join cats on cats.name = p.category_name
on conflict (store_id, name) do update
